...
export class PermissionProjectGuard implements CanActivate {
  ...
  canActivate(
    ...
    return this.store.select(state=>state.profile).pipe(
      ...
      mergeMap(state => forkJoin([of(state), this.api.apiProjectsRead(route.paramMap.get('id')!)])),
      map(([profileState, project]) => {
        if(project.owner?.id === profileState.data?.id || project.participants!.filter(p => p.id === profileState.data?.id).length > 0)
        {
          return true;
        }
