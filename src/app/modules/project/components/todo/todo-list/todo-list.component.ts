...
export class TodoListComponent implements OnInit {
  ...
  calcProgress() {
    const completedTodoItems = this.todoItems.reduce(
      (pre, todo) => pre + (todo.is_done ? 1 : 0),
      0
    );
    this.percentTodo = Math.round(
      (completedTodoItems / this.todoItems.length) * 100
    );
  }
}
